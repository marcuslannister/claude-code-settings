...
For remote operations, prefer `jj git push` / `jj git fetch`. Drop to raw `git` commands only when required for interoperability or a workflow that clearly depends on Git-specific behavior.
